...
import os
from src.machine_learning_ops.utils import save_object
...
class DataTransformation:
    ...
    def initiate_data_transformation(self,train_path,test_path):
        ...
        try:
            ...
            #print("col_name",input_features_test_df.columns)
            target_feature_test_df=test_df[target_col_name]

            logging.info("Applying Preprocessing on training and test dataframe")

            input_feature_train_arr=preprocessing_obj.fit_transform(input_features_train_df)
            #print("Array", input_feature_train_arr)
            input_feature_test_arr=preprocessing_obj.transform(input_features_test_df)

            train_arr=np.c_[input_feature_train_arr,np.array(target_feature_train_df)]
            test_arr=np.c_[input_feature_test_arr,np.array(target_feature_test_df)]

            logging.info(f"Saved prepprocessing Object for training and test dataframe in array")

            save_object(self.data_transformation_config.pre_processor_obj_file_path,obj=preprocessing_obj)

            return (train_arr,test_arr,self.data_transformation_config.pre_processor_obj_file_path)
